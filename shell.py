...
class FSShell():
    ...
    # implements repair (repair broken or disconnected server)
    def repair(self, server_number):

        # converting server_number to int
        try:
            server_number = int(server_number)
        except ValueError:
            print('Error: ' + server_number + ' not a valid Integer')
            return -1

        # check if server_number is valid
        if server_number < 0 or server_number >= fsconfig.NS:
            print("Error: Server number " + str(server_number) + " not in valid range [0, " + str(fsconfig.NS - 1) + "]")
            return -1

        # loop thru each block, reconstructing from other servers
        for block_number in range(0, fsconfig.TOTAL_NUM_BLOCKS//fsconfig.NS):

            # skipping lock bc it's in RSM server which never fails
            if block_number != (fsconfig.TOTAL_NUM_BLOCKS//fsconfig.NS - 1):
                block = bytearray(fsconfig.BLOCK_SIZE) # Byte array of all zeros
                for i in range(fsconfig.NS): # XORs all data together
                    if i != server_number:
                        other_data = self.RawBlocks.SingleGet(block_number, i)
                        block = bytearray(a ^ b for (a,b) in zip(block, other_data))

            # write block to new server
            self.RawBlocks.SinglePut(block_number, block, server_number)

            # displaying progress
            print("\rRepairing block: " + str(block_number) + " of " + str(fsconfig.TOTAL_NUM_BLOCKS//fsconfig.NS - 1), end='')
